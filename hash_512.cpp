...
/*uint8_t* XOR_KC(uint8_t *x, uint8_t *y) {
	uint8_t res[64];
	for(int i = 0; i < 64; ++i) {
		res[i] = x[i] ^ y[i];
	}
...
void cr_hash(struct hash &object, int flag) {     //flag = 0 ordinary hash, flag = 1 - hash without n and sigma, flag = 2 - g0
 ...
 /*printf("\nVEC_512\n");
  for (int i=0; i < 64; ++i) {
  	printf("%02x", VEC_512[i]);
  }
  printf("\n");
  printf("N_START\n");
  for (int i=0; i < 64; ++i) {
  	printf("%02x", object.n[i]);
  }
  printf("\n");*/
...
void finish(struct hash &object, uint8_t *result_hash, int fl) {
	...
	if (object.buffer_size  || fl) {
		...
		/*printf("OBJ1 = ");
		for(int j = 0; j < 64; ++j) {
			printf("%02x", object.cur_hash[j]);
		}
		printf("\n");*/
		memcpy(object.buffer, object.n, 64);
		/*printf("M=N = ");
		for(int j = 0; j < 64; ++j) {
			printf("%02x", object.n[j]);
		}*/
		cr_hash(object, 2);
...
/*void little_to_big(uint8_t *buf) {
	buf = ntohl(buf);
}*/
